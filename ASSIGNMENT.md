...
## Part 2 — Technical Readiness Note: Scheduling / Calendar Integration

**Workstream:** W2 — Integrations & Scheduling Systems
**Scope of this note:** the Epic-side scheduling integration that lets the agent search open slots, place holds, and create appointments on behalf of a patient. This is the integration most likely to slip (R2 = 16) and is the one the product promise depends on.

### Technical inputs required

1. **Epic API access path decided.** Options: (a) FHIR R4 Scheduling resources (Appointment, Slot, Schedule) via Epic on FHIR; (b) Epic App Orchard / Showroom integration with scheduling APIs; (c) HL7 v2 SIU messages. Default recommendation: **FHIR R4 for read + App Orchard scheduling APIs for write** — FHIR's write surface for Appointment is still maturing and uneven across Epic versions.
2. **Authentication artifacts.** OAuth 2.0 client ID + secret (or JWT signing key for backend auth); token endpoint; scoped permissions; non-prod + prod credentials separated.
3. **Epic environment access.** Non-prod (POC / TST) with realistic provider templates, Visit Types, and synthetic patients. "Realistic" is load-bearing — a bare sandbox hides the hard cases.
4. **Reference data.** Department IDs, Provider IDs, Visit Type → service-line mapping, appointment-status code list, cancel-reason code list.
5. **Scheduling rule book** (output of W1) — per-department overbooking policy, new-patient vs. established rules, insurance acceptance, telehealth eligibility, same-day policies.
6. **Data contract.** Field-level spec of what the agent sends, receives, and logs — with each field classified (PHI / non-PHI / configuration).
7. **Test patients + test providers** in non-prod, plus a named Epic analyst who can fix template issues day-of.

### Systems and teams to involve

| System / team | Why they are on the RACI |
|---|---|
| Hospital Epic Integration / Interface team | Owns Epic-side build, analyst resources, testing environment |
| Hospital IT Security | Auth review, network path, PHI classification, logging policy |
| Ajaia Solutions Engineer | Owns the integration layer between agent runtime and Epic |
| Agent platform vendor engineer | Tool/function schemas, retry semantics, idempotency keys |
| Hospital Privacy Officer | Defines the PHI boundary — what the agent sees vs. not |
| Patient Access Operations | Validates that what the agent can do matches clinic rules |
| Hospital Legal | Reviews any written disclosure sent to patients |

### Questions that must be answered before build begins

**Interface & contract**
1. Which Epic interface pattern are we using for write — FHIR Appointment, App Orchard scheduling, or HL7 v2 SIU? (affects effort and reliability)
2. Is autonomous booking allowed from day one, or is the pilot "hold + human confirms"? (materially changes risk)
3. What are the rate limits and SLAs for the chosen API? What happens during maintenance windows?

**Business rules**
4. Overbooking / double-booking policy per department — may the agent fill a blocked slot under any condition?
5. New patient vs. established — how is that determined at booking time, and who is eligible for which providers?
6. How do we match an inbound lead to an existing MRN? What if there's no match — create a pre-registration, or escalate?

**Data & privacy**
7. What is the minimum PHI the agent needs to function? Everything above that gets filtered at the integration layer.
8. What goes in logs, and for how long? Who can see transcripts that reference PHI?
9. Audit-log schema — what events, what fields, retention?

**Failure modes**
10. Idempotency key strategy for appointment creates — what key guarantees a retry is safe?
11. What happens when Epic is unavailable mid-conversation — queue, graceful degrade, or escalate?
12. Who owns reconciliation if an appointment is created in Epic but the conversation log lost the confirmation?

### What could go wrong technically

| Failure mode | Impact | Countermeasure |
|---|---|---|
| **Stale slot** — slot taken between agent's search and write | Patient told they're booked, then they're not | Two-phase: search → short-TTL hold → confirm; re-check on confirm |
| **Duplicate booking** on retry | Two appointments, angry provider | Idempotency key derived from (patient, slot, intent-id); server-side dedupe |
| **Rate-limit storm** at peak outreach | Cascading failures, missed appointments | Token-bucket client, backoff with jitter, outbound send scheduler |
| **Timezone / DST bug** | Patient arrives an hour early or late | All times stored UTC; display in patient's locale; DST test cases in QA |
| **Template drift** (Epic analyst changes a template mid-pilot) | Agent offers slots that don't exist | Daily template sync job + alert on schema diff |
| **Auth token expiry mid-flow** | Conversation dies at booking step | Proactive token refresh; retry on 401 once |
| **PHI leakage into logs** | Compliance incident | Structured logging with PHI fields masked at source; log review in W6 |
| **Partial success** — appointment created, confirmation SMS fails | Patient thinks they are not booked | Idempotent confirmation + reconciliation job + explicit human review queue |

### Definition of "ready to build"

Build does not start until **all** of the below are true. Anything short of this is a slip risk the tech lead should flag at Gate 1.

- [ ] Interface path chosen and documented (FHIR vs. App Orchard vs. HL7)
- [ ] Non-prod Epic environment accessible end-to-end from the Ajaia integration sandbox (auth works, a test call returns data)
- [ ] Signed data contract — every field classified, every PHI field justified
- [ ] Scheduling rule book signed by Patient Access + at least one clinical SME per service line
- [ ] PHI boundary approved by Privacy Officer in writing
- [ ] Idempotency + retry strategy reviewed by vendor engineer and Ajaia SE
- [ ] Failure-mode table above has a named owner for each countermeasure
- [ ] Test patients + test providers seeded in non-prod
- [ ] At least one working end-to-end scripted call (search → hold → confirm) demoed to the delivery lead
- [ ] P0/P1 issue severity ladder agreed + on-call rotation named for the integration

"Ready to build" is a **gate**, not a vibe. If you cannot tick every box, you are still de-risking, not building.
